...
~\r~ で同じ行を上書きするコマンド (~rsync --progress~ や ~curl~ の進捗
バー、あるいは手元で次のコマンド相当を実行) を試し、行が積み重ならず
同じ行が上書きされていくことを確認する。

#+begin_src sh
printf 'progress: %d%%\r' 10; sleep 0.3; printf 'progress: %d%%\r' 50; sleep 0.3; printf 'progress: %d%%\n' 100
#+end_src

** ~less~

~less~ で複数ページのファイル (このリポジトリの ~docs/design.org~ 等)
...
を確認する (design.org の方針どおり、プロセス実行中は対象外)。
...
- *パイプラインは末尾のプロセスのみ端末に接続する。*
  ...
  (docs/design.org の割り切り)。
- *プロセス実行中への IME 直接入力は対象外* (design.org 参照)。
