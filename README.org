#+title: emacs-spectreshell

eshell 向けの端末エミュレーション統合。[[https://ghostty.org/][ghostty]] の
...
- *[[https://codeberg.org/akib/emacs-eat][eat-eshell-mode]] との関係*:
...
設計の詳細は [[file:docs/design.org][docs/design.org]]、実装フェーズの経緯は
[[file:docs/implementation-plan.org][docs/implementation-plan.org]]、モジュール
境界の API 仕様は [[file:docs/module-api.org][docs/module-api.org]] を参照。

* 必要環境

- *Emacs 31 以降* (それより前のバージョンへの互換コードは書いていない)
- ビルドには *Zig 0.15.2* 系と、terminfo データベース生成用の
  ~tic~ (ncurses)、Info マニュアル生成用の ~makeinfo~ (texinfo) が必要
...
result/share/info/spectreshell.info     ; Info マニュアル
result/share/doc/spectreshell/          ; LICENSE / THIRD-PARTY-NOTICES.org
#+end_example
...
利用者向けの説明は Info マニュアル ([[file:docs/spectreshell.org][docs/spectreshell.org]] が
ソース) にもまとまっている。nix profile 等で ~share/info~ が ~INFOPATH~
に入る環境ならそのまま ~C-h i~ から、入らない環境でも
~C-u C-h i result/share/info/spectreshell.info~ で読める。

nix flake の devShell (~nix develop~) には Zig・ncurses・texinfo・emacs
(バッチテスト用) 一式が入っている。
...
~zig build~ には ~tic~ (ncurses) と ~makeinfo~ (texinfo) が必要。
~just build~ / ~just test~ /
~just test-el~ / ~just load-check~ / ~just nix-check~ に開発時によく使う
コマンドをまとめてある ([[file:Justfile][Justfile]] 参照)。
...
- マウス (SGR mouse mode): クリック・ドラッグ・ホイールスクロールを
  ...
  フォールバックする。tty フレーム (~emacs -nw~) では
  ~xterm-mouse-mode~ が有効でないとクリックが Emacs のイベントに
  ならないので、自動で有効にならない端末では利用者が
  ~(xterm-mouse-mode 1)~ する
  (グローバルなモードを勝手に切り替えないため spectreshell 側では
  有効にしない)
...
- *char モード非対応*: 全キー送信は v1 のスコープ外。semi-char モード
  で Emacs 側に残る ~C-c~ ・ ~C-x~ ・ ~M-x~ ・ ~C-u~ ・ ~C-g~ ・ ~M-O~ ・
  ~M-[~ を実行中プロセスへ送る手段はない (詳細は Info マニュアル)
...
詳細な手動確認手順は [[file:docs/manual-testing.org][docs/manual-testing.org]]
を参照。

* ライセンス

MIT License。詳細は [[file:LICENSE][LICENSE]] を参照。

ビルドされた ~libspectreshell.so~ には ghostty-vt をはじめとする
第三者ソフトウェア (いずれもパーミッシブライセンス) が静的リンクされる。
再配布に必要な許諾表示として、LICENSE と
[[file:THIRD-PARTY-NOTICES.org][THIRD-PARTY-NOTICES.org]] はビルド成果物の
~share/doc/spectreshell/~ に自動で同梱される。成果物ツリーごと再配布する
限り追加の作業は不要で、~libspectreshell.so~ 単体を取り出して再配布する
場合のみ両ファイルを別途同梱すること。
