...
(ert-deftest spectreshell-eshell-test-filter-ignores-detached-process ()
  "detach 後 (端末プロパティが nil) に届いた残余出力で filter が
エラーにならない。"
  (let* ((buf (generate-new-buffer " *spectreshell-filter-test*"))
         (proc (make-process :name "spectreshell-test-cat"
                             :command '("cat")
                             :buffer buf
                             :connection-type 'pipe)))
    (unwind-protect
        (progn
          (spectreshell-eshell--filter proc "leftover bytes")
          ;; 端末プロパティなしでは何も描画されない (エラーにもならない)。
...
(ert-deftest spectreshell-eshell-test-killed-process-recovers-prompt ()
  "kill-process によるシグナル死でも semi-char を抜けてプロンプトに戻る。"
  (spectreshell-eshell-test--with-eshell buf
    (spectreshell-eshell-test--send buf "sleep 30")
    (should (spectreshell-eshell-test--wait-until
             (lambda () (with-current-buffer buf
                          (and spectreshell-eshell--process
                               (process-live-p spectreshell-eshell--process))))))
    (kill-process (with-current-buffer buf spectreshell-eshell--process))
    (should (spectreshell-eshell-test--wait-for-command buf))
    (should (spectreshell-eshell-test--wait-until
             (lambda () (spectreshell-eshell-test--prompt-shown-p buf))))
    (with-current-buffer buf
...
(ert-deftest spectreshell-eshell-test-kill-buffer-during-command-releases-terminal ()
  "プロセス実行中にバッファを kill しても端末オブジェクトが解放される
(detach の「バッファ亡き後は直接 release」分岐)。"
  (spectreshell-eshell-test--with-eshell buf
    (spectreshell-eshell-test--send buf "sleep 30")
    (should (spectreshell-eshell-test--wait-until
             (lambda () (with-current-buffer buf
                          (and spectreshell-eshell--process
                               (process-live-p spectreshell-eshell--process))))))
    (let* ((proc (with-current-buffer buf spectreshell-eshell--process))
           (obj (process-get proc 'spectreshell-eshell-terminal)))
      (spectreshell-eshell-test--kill-buffer buf)
      ;; sentinel が走って detach が終わる (プロパティが消える) まで待つ。
      (should (spectreshell-eshell-test--wait-until
               (lambda () (null (process-get proc 'spectreshell-eshell-terminal)))))
      ;; モジュール端末は release 済みであるべき。
      (should-error (spectreshell--feed (spectreshell-term obj) "x")
...
(ert-deftest spectreshell-eshell-test-two-eshell-buffers-run-independently ()
  "eshell バッファ 2 つで並行に実行しても端末が混線しない
(spectreshell--current のバッファローカル分離)。"
  (spectreshell-eshell-test--with-eshell buf1
    (let ((buf2 (eshell t)))
      (unwind-protect
          (progn
            (spectreshell-eshell-test--send buf1 "*cat")
            (spectreshell-eshell-test--send buf2 "*cat")
            (dolist (buf (list buf1 buf2))
              (should (spectreshell-eshell-test--wait-until
                       (lambda () (with-current-buffer buf
                                    (and spectreshell-eshell--process
                                         (process-live-p spectreshell-eshell--process)))))))
            ;; 2 バッファは別々の端末オブジェクトを持つ。
            (should-not (eq (buffer-local-value 'spectreshell--current buf1)
                            (buffer-local-value 'spectreshell--current buf2)))
            ;; それぞれの入力エコーが自分のバッファにだけ現れる。
            (process-send-string
             (buffer-local-value 'spectreshell-eshell--process buf1) "first\n")
            (process-send-string
             (buffer-local-value 'spectreshell-eshell--process buf2) "second\n")
            (should (spectreshell-eshell-test--wait-until
                     (lambda () (with-current-buffer buf1
                                  (string-match-p "first" (buffer-string))))))
            (should (spectreshell-eshell-test--wait-until
                     (lambda () (with-current-buffer buf2
                                  (string-match-p "second" (buffer-string))))))
            ...
            (dolist (buf (list buf1 buf2))
              (process-send-string
               (buffer-local-value 'spectreshell-eshell--process buf) "\C-d")
              (should (spectreshell-eshell-test--wait-for-command buf))))
        (spectreshell-eshell-test--kill-buffer buf2)))))
...
不要になる\" という決定の裏付け)。"
  ...
  (spectreshell-eshell-test--with-eshell buf
    (let ((buffer-count-before (length (buffer-list))))
      ...
      (spectreshell-eshell-test--send
       buf (format "less %s"
                    (expand-file-name "docs/design.org" spectreshell-test--repo-root)))
      (should (spectreshell-eshell-test--wait-until
               (lambda () (with-current-buffer buf
                            (and spectreshell-eshell--process
                                 (process-live-p spectreshell-eshell--process))))))
  ...
  (with-temp-buffer
    (let ((obj (spectreshell-start (current-buffer) 24 80 #'ignore))
      ...
      (cl-letf (((symbol-function 'window-body-height) (lambda (_w) 30))
        ...
        (should (= (spectreshell-rows obj) 30))
        (should (= (spectreshell-cols obj) 100))
        (should (equal pty-size '(30 . 100)))
        ;; サイズ不変なら resize も ioctl も走らない。
        (setq pty-size nil)
        (spectreshell-eshell--window-size-change 'fake-window)
